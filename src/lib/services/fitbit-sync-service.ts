...
// The schema-cache error from the last attempt ("Could not find the
// 'avg_heart_rate' column ... in the schema cache") was Supabase's
// PostgREST layer serving a stale cache after the migration, not a real
// API problem with calories — same activity_and_fitness scope as
// steps, and the reload has since been triggered. Still isolated in its
// own try/catch since it's the one call never confirmed against a live
// response, so a genuine failure here shouldn't take steps/heart-rate/
// sleep down with it.
